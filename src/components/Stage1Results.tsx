...
export default function AuditResults({
  ...
  // IMPORTANT CHANGE: Agar audit result nahi mila, toh matlab spec audit nahi hua hai
  // Use "pending" ya "not-audited" status dena chahiye, default "correct" nahi
  const displaySpecs = originalSpecs.map(spec => {
    ...
    // Agar audit result nahi mila, toh spec audit nahi hua hai
    return {
      ...spec,
      auditResult: {
        specification: spec.spec_name,
        status: "not-audited", // YEH CHANGE KARO
        explanation: "Not audited",
        problematic_options: []
      },
      isAudited: false
    };
  ...
  const correctCount = displaySpecs.filter(s => 
    s.auditResult.status === "correct"
  ).length;
  const incorrectCount = displaySpecs.filter(s => 
    s.auditResult.status === "incorrect"
  ).length;
  const notAuditedCount = displaySpecs.filter(s => 
    s.auditResult.status === "not-audited"
  ).length;
  ...
  return (
    <div>
      ...
      {/* Summary Cards - Ab 3 cards honge */}
                ...
                {isNotAudited && (
                  <div className="mt-2 p-2 bg-white border border-gray-200 rounded text-xs">
                    <p className="text-gray-600 italic">
                      This specification was not processed in the audit.
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* Status Message */}
      {incorrectCount === 0 && notAuditedCount === 0 ? (
